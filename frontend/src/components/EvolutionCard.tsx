...
const EvolutionCard: React.FC<EvolutionCardProps> = ({ pokemon }) => {
  return (
    <div className="p-2 bg-gray-800 mt-8 ml-5 rounded-lg border border-gray-600 shadow-lg text-white font-pixel justify-center flex-shrink-0 mx-3 min-w-72 flex flex-col items-center">
      ...
      <div className="flex space-x-4 overflow-x-auto w-full">
        {pokemon.stages.map((stage, index) => (
          <div key={index} className="bg-gray-900 p-4 rounded-lg text-center w-60 flex-shrink-0">
            <img src={stage.image} alt={stage.stage} className="w-36 h-36 object-cover mb-3 rounded-sm mx-auto" />
